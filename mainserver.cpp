 ...
 * Daniel Artuso
 *
 * Main service that handles data transformations from a client
 * Creates a TCP connection with client on a child fork
 * Receives choices and string
 * For each choice send string to desired microservice on UDP
 * Receive back on UDP string and update
 *
 * Send back to client over
 */
...
/**
 * for each choice in choiceString
 * disaptch the sendRecvUDP with the correct port
 * @param choiceString
 * choiceString is received from client
 */
void dispatchUDP(const string &choiceString);

/**
 * calls the UDP microservices listening on the port passed
 * and receive the string back or timeout after not receiving for
 * 300ms
 * @param port
 */
void sendRecvUDP(int port);

/**
 * Inside a fork:
 * Call a function of the microservice
 * then exits with status code
 * @param port
 */
void callServer(int port);

/**
 * Handles forking and calling of both callServer and sendRecvUDP
 * @param port
 */
...
void sendRecvUDP(const int port) {
    cout << "Port " << port << endl;
